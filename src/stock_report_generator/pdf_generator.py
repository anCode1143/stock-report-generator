...
class PDFReportGenerator:
    ...
    def __init__(self, graphs_path=None):
        ...
        if graphs_path is None:
            ...
            self.graphs_path = os.path.join(
                os.path.dirname(os.path.dirname(os.path.dirname(__file__))), "graphs"
            )
        else:
        ...
        for element in soup.find_all(["h2", "h3", "p", "ul", "li"]):
            if element.name == "h2":
                ...
                if current_section:
                    ...
                    current_subsection = {"title": element.get_text().strip(), "content": []}
        ...
        for i, chart in enumerate(charts):
            ...
            if os.path.exists(chart_path):
                ...
                try:
                    ...
                    story.append(
                        Paragraph(
                            f"<i>Chart could not be loaded: {str(e)}</i>", self.styles["CustomBody"]
                        )
                    )
                    story.append(Spacer(1, 10))
            else:
                ...
                story.append(
                    Paragraph(
                        f"<i>Chart not found: {chart['filename']}</i>", self.styles["CustomBody"]
                    )
                )
                story.append(Spacer(1, 10))
    ...
    def convert_to_pdf(self, markdown_file, output_file=None):
        ...
        for section in content["sections"]:
            ...
            if (
                "EXECUTIVE SUMMARY" in section["title"].upper()
                or "INVESTMENT STRATEGY" in section["title"].upper()
                or "PRICE TARGETS" in section["title"].upper()
            ):
            ...
            for subsection in section["subsections"]:
                if (
                    "EXECUTIVE SUMMARY" in subsection["title"].upper()
                    or "INVESTMENT STRATEGY" in subsection["title"].upper()
                    or "PRICE TARGETS" in subsection["title"].upper()
                ):
                    section_needs_pagebreak = True
                    break
