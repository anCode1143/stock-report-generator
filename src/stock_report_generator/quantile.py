...
class QuantileForecaster:
    ...
    def __init__(self, ticker, forecast_horizon=6, alpha=0.01, window=20):
        ...
        self.output_path = os.path.join(
            self.project_root, "graphs", f"{self.ticker}_quantile_forecast.png"
        )
    ...
    def _load_and_prepare_data(self):
        ...
        df["future_high_close"] = (
            df["Close"]
            .rolling(window=self.forecast_horizon, min_periods=1)
            .max()
            .shift(-self.forecast_horizon + 1)
        )
        df["future_low_close"] = (
            df["Close"]
            .rolling(window=self.forecast_horizon, min_periods=1)
    ...
    def plot_results(self, backtest_preds, forecast_pred):
        ...
        forecast_dates = [
            last_datetime + dt.timedelta(days=i) for i in range(1, self.forecast_horizon + 1)
        ]

        forecast_range = forecast_pred.iloc[0]
        plt.fill_between(
            forecast_dates, forecast_range[0.05], forecast_range[0.95], color="lightblue", alpha=0.3
        )
        plt.fill_between(
            forecast_dates,
            forecast_range[0.15],
        ...
        plt.fill_between(
            forecast_dates, forecast_range[0.25], forecast_range[0.75], color="royalblue", alpha=0.5
        )
    ...
    def run(self):
        ...
        if len(self.df) <= self.window:
            print(
                f"Error: Not enough data for {self.ticker} to perform backtest. "
                f"Need > {self.window} rows, have {len(self.df)}."
            )
            return None
